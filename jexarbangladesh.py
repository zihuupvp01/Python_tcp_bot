#ONLY ONE THE KING OF COMMITTIY : Zihuu_pvp
#INSTAGRAM : Zihuu_pvp
#YOUTUBE : @Zihuu_pvp
#TELEGRAM : @Zihuu_pvp
...
def send_vistttt(uid):
    try:
        ...
        api_url = f"https://client.us.freefiremobile.com/GetPlayerPersonalShow"
...
def newinfo(uid):
    try:
        url = f"https://xp-full-info-v4.vercel.app/player-info?uid={uid}&region={region}"
        response = requests.get(url)
...
def attack_profail(player_id):
    url = f"https://client.us.freefiremobile.com/GetPlayerPersonalShow"
...
def send_likes(uid):
    likes_api_response = requests.get(f"https://like-xp-v12.vercel.app/like?server_name={region}&uid={uid}&key=xp")
...
[FFFFFF]Hello! 😊  
[FFFFFF]Tᴄʟᴘ_ʙᴏᴛ:  
[FFFFFF]ᴇᴍᴏᴊɪ sᴇɴᴛ ᴋᴏʀᴋ! 😊  
[1E90FF]╠══════════════════════════╣
[FFFFFF]ʀᴇᴀᴅʏ ғᴏʀ ᴄᴏᴍᴍᴀɴᴅ? 🤖  
[FFFFFF]Tech support ke liye developer ko message karo 📩  
[FFD700]TELEGRAM : [FFFF00]@Zihuu_pvp
[1E90FF]╚══════════════════════════╝""", idinv
                        )
                )
                senthi = False
...
[b][i][A5E2CFٍ]Instagram: @Zihuu_pvp[/b]

[b][c][1E90FF] Telegram: @Zihuu_pvp""",uid
                            )
                        )
                else:
                    pass  
...
[b][i][A5E2CFٍ]telegram: Zɪʜᴜᴜ_ᴘᴠᴘ[/b]

[b][c][1E90FF]Instagram : Zɪʜᴜᴜ_ᴘᴠᴘ[FFFFFF]
...
[C][B][FF6347] Developer    Zɪʜᴜᴜ_ᴘᴠᴘ""", uid
                    )
                )
            ...
            if "1200" in data.hex()[0:4] and b"/x" in data:
                try:
                    ...
                    if len(command_split) > 1:
                        ...
                        clients.send(
                            self.GenResponsMsg(
                                f"[C][B][1E90FF]Spam Request Chaʟᴜ ʜᴏɪᴄᴇi...!!!\n"                              , uid
                            )
                        )                            
            ...
            if "1200" in data.hex()[0:4] and b"WELCOME TO [FFFFF00]Zihuu_pvp [ffffff]BOT" in data:
            	pass
            else:
    ...
    f"[B][FFA500]• Bio: [FFFFFF]{bio}\n"
	            ...
	            if "1200" in data.hex()[0:4] and b"/help[ffffff]" in data:
	                ...
	                clients.send(
	                    self.GenResponsMsg(
		                        f"""[B][C][FFFF00] WELCOME TO [ffffff]Zihuu_pvp BOT  !! 
...
if __name__ == "__main__":
    try:
        client_thread = FF_CLIENT(
            id="4132198722",
            password="F704250D32D20DFBE73847683CC693B3D1EE325805F0FA0C7F6828BBDCBC84C4"
        )
        restart_program()
    except Exception as e:
